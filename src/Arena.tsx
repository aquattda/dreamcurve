...
function ArenaHome({state,onRetry,retrying}:{state:ArenaState;onRetry:()=>void;retrying:boolean}) {
  ...
  const [trade,setTrade]=useState<{marketId:string;side:Side}|null>(null);
  ...
  return <>
    <div className="arena-layout">
      ...
      <section className="market-detail">
        ...
        <div className="agent-grid">{forecasts.map(f=><AgentCard key={f.agentId} forecast={f} allowTrade={allowTrade} unavailableLabel={market.source==='live'?'Trading paused':'Illustration only'} onTrade={side=>setTrade({marketId:market.id,side})}/>)}</div>
      </section>
    </div>
    {trade?.marketId===market.id&&market.source==='live'?<TradeModal key={`${market.id}:${account??'disconnected'}`} funding={wallet} market={market} side={trade.side} account={account} onConnect={connect} onClose={()=>setTrade(null)}/>:null}
  </>;
}
