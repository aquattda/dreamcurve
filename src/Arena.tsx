...
import { claimTestUsdc, connectWallet, loadTestUsdcBalance, placeStake } from './wallet-bridge';
...
function ArenaHome({state,onRetry,retrying}:{state:ArenaState;onRetry:()=>void;retrying:boolean}) {
  ...
  return <>
    ...
    {trade?<TradeModal key={account??'disconnected'} market={market} side={trade} account={account} onConnect={connect} onClose={()=>setTrade(null)}/>:null}
  </>;
}
...
export default function Arena(){
  ...
  const connect=useCallback(async()=>{
    ...
    try{const balance=await loadTestUsdcBalance(next);if(accountRef.current===next){setTestUsdc(balance);setFundingStatus('')}}catch(e){setFundingStatus(walletMessage(e,'Could not read the tUSDC balance.'))}
  },[]);
  const refreshTestUsdc=useCallback(async()=>{
    if(!account)return;
    try{const balance=await loadTestUsdcBalance(account);if(accountRef.current===account)setTestUsdc(balance)}catch(e){if(accountRef.current===account)setFundingStatus(e instanceof Error?e.message:'Could not read the tUSDC balance.')}
  },[account]);
  const requestTestUsdc=useCallback(async()=>{
    ...
    }catch(e){setFundingStatus(walletMessage(e,'The tUSDC faucet request failed.'))}finally{fundingLock.current=false;setFundingBusy(false)}
  },[account]);
  useEffect(()=>{
  ...
  return <WalletContext.Provider value={{account,connect,testUsdc,fundingBusy,fundingStatus,fundingHash,refreshTestUsdc,requestTestUsdc}}><div className="app-shell"><AppHeader account={account} onConnect={connect} mode={mode}/><DataBanner state={state} error={error||walletError} mode={mode}/><main id="main-content" className="app-main">{<Routes><Route index element={state?<ArenaHome state={state} onRetry={retry} retrying={retrying}/>:<section className="loading-state">Opening the arena…</section>}/><Route path="agents" element={state?<AgentsPage state={state}/>:<section className="loading-state">Loading agents…</section>}/><Route path="leaderboard" element={state?<Scorecard state={state}/>:<section className="loading-state">Loading scorecard…</section>}/><Route path="portfolio" element={<PortfolioRoute/>}/><Route path="*" element={<EmptyState live={false}/>}/></Routes>}</main><footer className="app-footer"><span>DreamCurve · Somnia Shannon testnet</span><Link to="/methodology">Methodology</Link><button onClick={load}>Refresh data</button></footer></div></WalletContext.Provider>
}
